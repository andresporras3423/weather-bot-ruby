...
RSpec.describe Botclient do
  ...
  describe 'tests method update_format' do
    it 'test when format is not valid' do
      expect(botclient.chatbot.update_format(Msg.new("/format #$1124"))).to eql("invalid format")
    end
  ...
  describe 'tests method update_interval' do
    it 'test when interval is not valid' do
      expect(botclient.chatbot.update_interval(Msg.new("/interval #$1124"))).to eql("invalid interval")
    end
  ...
  describe 'tests method capture_error' do
    ...
    it 'test when weather_coord and invalid params' do
      expect{botclient.capture_error([1000, -1000], 'weather_coord')}.to output("a problem has occurred: Something was wrong!\n").to_stdout
    end
    ...
    it 'test when weather_zip and invalid params' do
      expect{botclient.capture_error(["11111"], 'weather_zip')}.to output("invalid data by error: Something was wrong!\n").to_stdout
    end
    ...
    it 'test when weather_city and invalid params' do
      expect{botclient.capture_error(["lalaland"], 'weather_city')}.to output("invalid data by error: Something was wrong!\n").to_stdout
    end
